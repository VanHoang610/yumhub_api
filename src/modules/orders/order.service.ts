...
export class OrderService {
    ...
    async sortHistory(id: string, who: number) {
        try {
            ...
            switch (who) {
                case 1:
                    orderSort = await this.orderModel.find({ customerID: id }).sort({ timeBook: 1 }).exec();
                    break;
                case 2:
                    orderSort = await this.orderModel.find({ merchantID: id }).sort({ timeBook: 1 }).exec();
                    break;
                case 3:
                    orderSort = await this.orderModel.find({ shipperID: id }).sort({ timeBook: 1 }).exec();
                    break;
                case 4:
                    orderSort = await this.orderModel.find({ voucherID: id }).sort({ timeBook: 1 }).exec();
                    break;
                default:
