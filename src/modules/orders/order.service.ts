...
export class OrderService {
  constructor(
    ...
    @InjectModel(DetailOrder.name) private detailOrderModel: Model<DetailOrder>,
  ...
  async revenueFoodAndDelivery(month: string) {
    try {
      ...
      const endOfMonthWithTime = set(endOfMonthDate, {
        hours: 23,
        minutes: 59,
        seconds: 59,
        milliseconds: 999,
      ...
      const createAggregatePipeline = (startOfMonth, endOfMonth, statusID) => [
        {
          $match: {
            timeBook: { $gte: startOfMonth, $lte: endOfMonth },
            status: statusID,
          },
        },
        {
        ...
        },
        {
          $group: {
            ...
            totalFood: { $sum: '$priceFood' },
            totalShip: { $sum: '$deliveryCost' },
            totalMerchant: { $sum: '$revenueMerchant' },
            totalShipper: { $sum: '$revenueDelivery' },
            totalVoucher: {
              $sum: {
                $cond: [
                  { $eq: ['$status', DeliveredID] },
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const orderSuccessPipeline = createAggregatePipeline(
          startOfMonth,
          endOfMonth,
          DeliveredID,
        );
        const orderFakePipeline = createAggregatePipeline(
          startOfMonth,
          endOfMonth,
          fakeOrderID,
        );

        const orderSuccessPromise = this.orderModel
          .aggregate(orderSuccessPipeline)
          .exec();
        const orderFakePromise = this.orderModel
          .aggregate(orderFakePipeline)
          .exec();
      ...
      for (let i = 0; i < results.length; i += 2) {
        ...
        const totalRevenue =
          (orderSuccess.totalRevenue || 0) + (orderFake.totalRevenue || 0);
        const totalFood =
          (orderSuccess.totalFood || 0) + (orderFake.totalFood || 0);
        const totalShip = orderSuccess.totalShip || 0; // Only successful orders have shipping cost
        const totalProfitMerchant =
          (orderSuccess.totalMerchant || 0) + (orderFake.totalMerchant || 0);
        const totalProfitShipper = orderSuccess.totalShipper || 0; // Only successful orders have shipper profit
        const totalVoucher =
          (orderSuccess.totalVoucher || 0) + (orderFake.totalVoucher || 0);

        revenueData.push({
          totalRevenue: totalRevenue,
          totalFood: totalFood,
          totalShip: totalShip,
          totalMerchant: totalProfitMerchant,
          totalShipper: totalProfitShipper,
      ...
      return {
        ...
        lastMonth: revenueData[1] || {},
        thisMonth: revenueData[0] || {},
      };
    } catch (error) {
  ...
  async getAllOrderStatus() {
    try {
      ...
      if (!orderStatus)
        throw new HttpException('Not found orderStatus', HttpStatus.NOT_FOUND);
