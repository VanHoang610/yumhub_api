...
export class AdminService {
  ...
  async changePass(id: string, passOld: string, passNew: string) {
    try {
      ...
      if (!compare)
        throw new HttpException('Password Fail', HttpStatus.UNAUTHORIZED);
  ...
  async createAdmin(admin: CreateAdminDto) {
    try {
      ...
      const checkPhone = await this.adminModel.findOne({
        phoneNumber: admin.phoneNumber,
      });
      if (checkPhone)
        throw new HttpException('SDT đã tồn tại', HttpStatus.BAD_REQUEST);
      const checkUsername = await this.adminModel.findOne({
        userName: admin.userName,
      });
      if (checkUsername)
        throw new HttpException('Username đã tồn tại', HttpStatus.BAD_REQUEST);
  ...
  async updateAdmin(id: string, admin: updateAdminDto) {
    try {
      const updateAdmin = await this.adminModel.findByIdAndUpdate(id, admin, {
        new: true,
      });
  ...
  async checkRole() {
    try {
      return { result: true, role: "You are manager" };
    } catch (error) {
      return { result: false, role: error.message };
    }
  }
}
